...
interface SearchMessageGifsBody {
  query?: unknown;
  limit?: unknown;
  pos?: unknown;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!apiKey) {
      return jsonResponse({ error: "GIF search is not configured" }, 503, origin);
    }

    const body = await parseJsonBody<SearchMessageGifsBody>(req);
    ...
    console.error("search-message-gifs failed", error);
    return jsonResponse(
