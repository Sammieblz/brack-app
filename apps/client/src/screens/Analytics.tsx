...
import { AnalyticsStatsSkeleton } from "@/components/skeletons/AnalyticsSkeleton";
...
const Analytics = () => {
  const { user } = useAuth();
  const { books, loading: booksLoading } = useBooks(user?.id);
  const {
    readingProgress,
    genreData,
    weeklyReading,
  ...
  const avgMinutesPerDay =
    Math.round(totalMinutes / readingProgress.length) || 0;
    ...
    <MobileLayout>
      ...
      <div className="app-page space-y-6">
        ...
        >
          {error && (
            <LoadingError
              message="Analytics could not be refreshed. Your reading data is safe."
          ...
          <Tabs defaultValue="overview" className="w-full">
            ...
            <TabsContent value="detailed" className="space-y-6">
              ...
              {/* Completion Rate */}
              {!loading && completionRate.length > 0 ? (
              ...
              {/* Streak Timeline */}
              {!loading && streakTimeline.length > 0 ? (
            ...
            <TabsContent value="insights" className="space-y-6">
              ...
              {/* Time Distribution */}
              {!loading && timeDistribution.length > 0 ? (
