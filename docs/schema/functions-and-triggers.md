...
| `auth_email_exists(p_email text)` | Legacy backend RPC | Normalized signup email | Boolean | Read-only predicate over `auth.users` retained with `auth-email-availability` for rollback/older clients; not used by current signup. `SECURITY DEFINER` with a fixed search path; execute is revoked from `PUBLIC`, `anon`, and `authenticated` and granted only to `service_role`. |
