...
for (const route of ["/turnstile.html", "/turnstile"]) {
  const escapedRoute = route.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const rule = pagesHeaders.match(
    new RegExp(`^${escapedRoute}\\r?\\n((?:[ \\t]+.*(?:\\r?\\n|$))+)`, "m")
  )?.[0];

  assert.ok(rule, `The ${route} response header rule is missing.`);
  assert.match(
    rule,
    /Cache-Control: no-store, max-age=0/,
    `The ${route} response must use a no-store policy.`
  );
  assert.match(
    rule,
    /Content-Security-Policy:.*frame-ancestors/,
    `The ${route} response must retain the Turnstile bridge CSP.`
  );
}
assert.match(
  pagesHeaders,
